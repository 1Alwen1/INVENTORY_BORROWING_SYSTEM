        ...
        <div class="container-fluid">
			<table class="table table-hover table-striped table-bordered" id="list">
				...
				<tbody>
					<?php
					$i = 1;
						$qry = $conn->query("SELECT i.*,c.name as `category` from `item_list` i inner join category_list c on i.category_id = c.id where c.delete_flag = 0 and i.delete_flag = 0 and i.category_id NOT IN (SELECT category_id FROM accessory_list WHERE delete_flag = 0) order by i.`LabID` asc ");
					...
					?>
						...
						<td class="px-1 py-2"><?= $row['category'] ?></td>
							...
							<td class="px-1 py-2"><?= $row['batch'] ?></td>
													<td class="px-1 py-2"><?= $row['brand'] ?? '' ?></td>
						<td class="px-1 py-2"><?= $row['model'] ?? '' ?></td>
						<td class="px-1 py-2"><?= $row['supplier_name'] ?? '' ?></td>
						<td class="px-1 py-2"><?= $row['serial_id'] ?? '' ?></td>
