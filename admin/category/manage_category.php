...
<div class="container-fluid">
	<form action="" id="category-form">
		...
		<div class="form-group">
			...
			<select name="type" id="type" class="form-control form-control-sm rounded-0" required="required">
				<option value="item" <?= isset($type) && $type == 'item' ? 'selected' : '' ?>>Item</option>
				<option value="accessory" <?= isset($type) && $type == 'accessory' ? 'selected' : '' ?>>Accessory</option>
			</select>
		</div>
		<div class="form-group">
			...
			$.ajax({
				...
				success:function(resp){
					if(typeof resp =='object' && resp.status == 'success'){
						...
						uni_modal("<i class='fa fa-th-list'></i> Category Details ","category/view_category.php?id="+resp.cid)
