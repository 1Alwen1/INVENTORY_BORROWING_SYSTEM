				...
				<tbody>
					<?php
						...
						while($row = $qry->fetch_assoc()):
							...
							// Fetch combined Category/LabID for items and accessories
							$cat_labid_sql = "
								SELECT DISTINCT CONCAT(c.name, '/', i.LabID) as cat_labid
								FROM `record_item_list` ril
								INNER JOIN `item_list` i ON ril.item_id = i.id
								INNER JOIN `category_list` c ON i.category_id = c.id
								WHERE ril.record_id = '{$row['id']}' AND c.delete_flag = 0 AND i.delete_flag = 0
								UNION
								SELECT DISTINCT CONCAT(c.name, '/', a.batch) as cat_labid
								FROM `record_accessory_list` ral
								INNER JOIN `accessory_list` a ON ral.accessory_id = a.id
								INNER JOIN `category_list` c ON a.category_id = c.id
								WHERE ral.record_id = '{$row['id']}' AND c.delete_flag = 0 AND a.delete_flag = 0
								ORDER BY cat_labid
							";
							$cat_labid_qry = $conn->query($cat_labid_sql);
							$cat_labid_arr = [];
							while ($cl_row = $cat_labid_qry->fetch_assoc()) {
								$cat_labid_arr[] = $cl_row['cat_labid'];
							}
							$cat_labid_str = !empty($cat_labid_arr) ? implode(', ', $cat_labid_arr) : '---';
					?>
						<tr>
							<td class="px-2 py-1"><?= $meta['borrower_name'] ?? "---" ?></td>
							<td class="px-2 py-1"><?= $meta['borrower'] ?? "---" ?></td>
							<td class="px-2 py-1"><?= $cat_labid_str ?></td>
							...
							<td align="center">
								 <button type="button" class="btn btn-flat p-1 btn-default btn-sm dropdown-toggle dropdown-icon" data-toggle="dropdown">
				                  ...
				                  <div class="dropdown-menu" role="menu">
				                    ...
				                    <div class="dropdown-divider"></div>
				                    <?php if(!isset($meta['returned_date']) || empty($meta['returned_date'])): ?>
				                    <a class="dropdown-item return_data" href="javascript:void(0)" data-id="<?php echo $row['id'] ?>"><span class="fa fa-undo text-success"></span> Return</a>
				                    <div class="dropdown-divider"></div>
				                    <?php endif; ?>
...
<script>
	$(document).ready(function(){
		...
		$('.return_data').click(function(){
			_conf("Are you sure to return this borrow record?","return_borrow",[$(this).attr('data-id')])
		})
	...
	function return_borrow($id){
		start_loader();
		$.ajax({
			url:_base_url_+"classes/Master.php?f=return_borrow",
			method:"POST",
			data:{id: $id},
			dataType:"json",
			error:err=>{
