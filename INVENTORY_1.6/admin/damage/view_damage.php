...
if(isset($_GET['id']) && $_GET['id'] > 0){
	$qry = $conn->query("SELECT d.*, i.LabID as icode, c.name as `category`, c.image_path from `damage_list` d inner join `item_list` i on d.item_id = i.id inner join category_list c on i.category_id = c.id where d.id = '{$_GET['id']}' and c.`delete_flag` = 0 and i.`delete_flag` = 0 ");
    if($qry->num_rows > 0){
...
<div class="container-fluid">
	...
	<dl>
		...
		<dt class="text-muted">Item LabID</dt>
