						...
						<tr>
							...
							<td class="px-2 py-1 text-center">
								<div class="border rounded">
									<img src="<?= validate_image($row['image_path']) ?>" alt="<?= $row['name'] ?>" class="category-img">
								</div>
							</td>
							...
							<td class="px-2 py-1 text-center">
                                <?php if($row['type'] == 'item'): ?>
                                    <span class="badge badge-primary px-3 rounded-pill">Item</span>
                                <?php else: ?>
                                    <span class="badge badge-info px-3 rounded-pill">Accessory</span>
                                <?php endif; ?>
                            </td>
							<td class="px-2 py-1 text-center">
                                <?php if($row['status'] == 1): ?>
...
<script>
	$(document).ready(function(){
		...
		$('#create_new').click(function(){
			uni_modal("<i class='far fa-plus-square'></i> Add New Category ","category/manage_category.php", "modal-lg")
		})
		$('.edit-data').click(function(){
			uni_modal("<i class='fa fa-edit'></i> Add New Category ","category/manage_category.php?id="+$(this).attr('data-id'), "modal-lg")
		})
