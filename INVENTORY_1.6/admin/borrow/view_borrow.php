					...
					<div class="row">
						<div class="col-lg-7 col-md-10 col-sm-12 col-12">
							<div id="item-list" class="list-group mx-4">
								<?php if(!empty($id)): ?>
								<?php 
								$item_list_sql = "SELECT i.*, c.name as `category` FROM `item_list` i inner join `category_list` c on i.category_id = c.id where i.id IN (SELECT `item_id` FROM `record_item_list` where `record_id` = '{$id}' ) order by c.name, i.LabID asc";
									...
									<div class="list-group-item list-group-item-action">
										<div class="d-flex w-100 align-items-center">
											<div class="col-auto flex-grow-1">
												...
												<span><?= $row['category'] . " | " . $row['LabID'] ?></span>
											</div>
										</div>
									</div>
								<?php endforeach; ?>
