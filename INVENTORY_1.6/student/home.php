            ...
            <div class="card card-outline card-primary">
              <div class="card-header">
                ...
                <div class="card-tools">
                  <a href="../classes/Login.php?f=logout_student" class="btn btn-danger btn-sm">Logout</a>
                </div>
              </div>
              <div class="card-body">
                <div class="row">
                        ...
                        <table class="table table-bordered table-striped" id="reservation-table">
                          ...
                          <tbody>
                            ...
                            while($row = $reservations->fetch_assoc()){
                              $status = ['Pending','Borrowed','Borrowed','Returned','Declined'][$row['status']] ?? 'Unknown';
                              $items = $conn->query("SELECT CONCAT(c.name, '/', i.LabID) as cat_labid FROM item_list i INNER JOIN category_list c ON i.category_id = c.id WHERE i.id IN (SELECT item_id FROM reservation_items WHERE reservation_id = {$row['id']})");
                              $item_list = [];
                              while($i = $items->fetch_assoc()) $item_list[] = $i['cat_labid'];
